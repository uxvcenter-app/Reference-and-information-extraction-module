...
class ExampleUnitTest {
    ...
    @Test
    fun testArbitraryUserTypedTextExtraction() {
        val sample = "In 2024, John Smith visited Apple Inc. in Chicago and spent $1,500.50 on 25% discounted items. See [1] or visit https://www.apple.com."
        val options = ExtractionOptions(
        ...
        val dateItem = results.find { it.category == ExtractionCategory.TEMPORAL_DATE }
        assertNotNull("Date should be extracted from user text", dateItem)

        val personItem = results.find { it.subCategory == "PERSON" }
        assertNotNull("Person 'John Smith' should be extracted from user text", personItem)
        assertEquals("John Smith", personItem?.text)

        val orgItem = results.find { it.subCategory == "ORGANIZATION" }
        assertNotNull("Organization 'Apple Inc.' should be extracted from user text", orgItem)
        assertEquals("Apple Inc.", orgItem?.text)

        val locItem = results.find { it.subCategory == "LOCATION" }
        assertNotNull("Location 'Chicago' should be extracted from user text", locItem)
        assertEquals("Chicago", locItem?.text)

        val currItem = results.find { it.subCategory == "FINANCIAL_CURRENCY" }
        assertNotNull("Currency '$1,500.50' should be extracted from user text", currItem)
        assertEquals("$1,500.50", currItem?.text)

        val pctItem = results.find { it.subCategory == "PERCENTAGE" }
        assertNotNull("Percentage '25%' should be extracted from user text", pctItem)

        val bracketItem = results.find { it.subCategory == "NUMERIC_CITATION" }
        assertNotNull("Bracket citation '[1]' should be extracted", bracketItem)

        val urlItem = results.find { it.subCategory == "WEB_URL" }
        assertNotNull("URL should be extracted", urlItem)
    }
}
